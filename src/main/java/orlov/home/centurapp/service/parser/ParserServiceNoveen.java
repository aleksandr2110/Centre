...
@Service
@Slf4j
public class ParserServiceNoveen extends ParserServiceAbstract {

    private final String SUPPLIER_NAME = "noveen";
    private final String SUPPLIER_URL = "https://noveen.com.ua/";
    private final String DISPLAY_NAME = "189-NOVEEN";
    private final String URL_PART_PAGE = "?page=";
    private final String MANUFACTURER_NAME = "Noveen";
    ...
    public ParserServiceNoveen(AppDaoService appDaoService, OpencartDaoService opencartDaoService, ScraperDataUpdateService scraperDataUpdateService, TranslateService translateService, FileService fileService, UpdateDataService updateDataService) {
    ...
    public List<CategoryOpencart> getSiteCategories(SupplierApp supplierApp) {
        ...
        if (Objects.nonNull(doc)) {
            ...
            List<CategoryOpencart> mainCategories = doc.select("div#footer a[href^=catalog/]")
                    .stream()
                    .map(ec -> {
                        String partUrl = ec.select("a").attr("href");
                        String url = SUPPLIER_URL.concat(partUrl);
                    ...
                    if (Objects.nonNull(doc)) {
                        ...
                        while (countPage != -1) {
                            try {
                                ...
                                Elements elementsProduct = doc.select("div.item.product.sku");
                                log.info("elementsProduct: {}", elementsProduct.size());

                                elementsProduct
                                        .stream()
                                        .peek(ep -> {
                                            String partUrlProduct = ep.select("a.name").attr("href");
                                            String urlProduct = SUPPLIER_URL.concat(partUrlProduct);
                                            log.info("Product url: {}", urlProduct);

                                            String title = ep.select("a.name").text().trim();
                                            log.info("Product title: {}", title);
                                            String sku = ep.select("span[data-product]").attr("data-product");
                                            String model = generateModel(sku, "0000");
                                            log.info("Product sku: {}", sku);

                                            Elements priceElement = ep.select("a.price");
                                            String price = "0.0";
                                ...
                                List<Integer> paginationPages = doc.select("div.bx-pagination ul li")
                                        .stream()
                ...
                .peek(p -> {
                    ...
                    if (Objects.nonNull(webDocument)) {
                        try {
                            ...
                            String name = webDocument.select("*[data-product]").text().trim();
                            log.info("name: {}", name);
                            ...
                            Elements tableAttr = webDocument.select("div#elementProperties tbody tr");
                            ...
                            Elements imagesElement = webDocument.select("div.pictureSlider div.item");
                            ...
                            if (imagesElement.size() > 0) {
                                ...
                                List<ImageOpencart> productImages = imagesElement
                                        .stream()
                                        .map(ie -> {
                                            String url = ie.select("img").attr("src");
    ...
    public String getDescription(Document doc) {
        Elements descElement = doc.select("div.changeDescription");
